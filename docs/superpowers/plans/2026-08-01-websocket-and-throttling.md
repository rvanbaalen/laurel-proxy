...
/**
 * Deterministic clock for single-consumer rate arithmetic.
 * NOTE: sleep() sums durations rather than modelling overlap, so this helper
 * CANNOT distinguish concurrent from sequential consumers. The shared-pipe
 * concurrency test uses Vitest fake timers instead — see below.
 */
